...
```python
class Booking:
    def __init__(self, user_id: str, resource_id: str, time_slot: TimeSlot):
        self.user_id = user_id
        self.resource_id = resource_id
...
Щоб це працювало в Python, Value Object має визначати `__eq__` і `__hash__`. `@dataclass(frozen=True)` робить це автоматично.
...
### Як забезпечити в Python

```python
@dataclass(frozen=True)  # frozen=True блокує зміну полів після створення
class Money:
    amount: Decimal
    currency: str
...
`add` не змінює поточний об'єкт — повертає новий. Як `str.upper()` в Python повертає новий рядок, а не змінює існуючий.
...
class Email:
    ...
    def __post_init__(self):
        ...
        # нормалізуємо до lowercase, щоб не було дублікатів
...
### Де закінчується Value Object і починається Service?

Value Object містить логіку, яка стосується **його власних даних**:
- `TimeSlot.overlaps(other)` — порівняння двох слотів, потрібні тільки їхні поля
- `Money.add(other)` — додавання, потрібні тільки суми і валюти
- `Email.domain()` — витяг домену з адреси

Якщо логіка потребує зовнішніх залежностей (БД, інші сервіси) — це вже не Value Object, а [Domain Factory](domain-factory.md) або Domain Service.
...
Ні. [DTO](dto.md) — об'єкт для передачі даних через кордон між шарами. Value Object — доменна концепція з поведінкою та інваріантами. DTO не має логіки. Value Object має. DTO живе на межі шарів. Value Object живе в домені.
