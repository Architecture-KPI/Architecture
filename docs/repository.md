...
Доменна модель не повинна знати, де і як вона зберігається. Це основа DIP і шарової архітектури. Але домену потрібна можливість «зберегти бронювання» і «знайти бронювання за ID». 
...
Ключове слово: **агрегати**. Repository працює з доменними сутностями, а не з таблицями, рядками або довільними проєкціями.
...
Так. Якщо `BookingReadRepository` обслуговує і сторінку бронювань користувача, і адмін-дашборд, і календар — це знову різні споживачі з різними причинами для змін.
...
Ні. Repository створюється для доменних об'єктів. Якщо `TimeSlot` — Value Object всередині `Booking`, йому не потрібен окремий репозиторій. Він зберігається і завантажується разом з `Booking`.

### «Всю роботу з БД треба робити через Repository»

Тільки роботу з **доменними моделями**. Запити для UI, звіти, аналітика, пошук — це не Repository. Це Query Service, або прямий запит в БД з Query Handler'а.
