...
### Асинхронна: через події

Один модуль публікує [подію](events.md), інші підписуються й реагують:
...
Часто наші додатки інтегруються з зовнішніми системами: платіжний сервіс, сервіс нотифікацій, інвентаризація, аналітика. Ці системи мають **свій життєвий цикл, свій контекст і своє API**, яке часто мало схоже на те, що потрібно нашій бізнес-логіці.

Anti-Corruption Layer — це компонент, який **ховає за собою деталі взаємодії** з зовнішньою системою (або іншим модулем), надаючи нашій бізнес-логіці зрозуміле їй API.

### Структура ACL

ACL складається з кількох частин, кожна з яких має чітку роль:

```mermaid
graph LR
    BL["Бізнес-логіка"] -->|"наш інтерфейс"| Adapter["Адаптер"]
    Adapter -->|"виклики в чужих термінах"| Facade["Фасад (опціонально)"]
    Facade -->|"HTTP / SDK"| External["Зовнішня система"]
    Adapter -.->|"трансляція даних"| Translator["Транслятор"]
```

| Компонент | Роль | Терміни |
|-----------|------|---------|
| **Інтерфейс** | Контракт, який бізнес-логіка очікує від інтеграції. Виражений в **наших** доменних термінах | Наші |
| **Фасад** (опціонально) | Обгортка навколо чужого API, що покращує його читабельність. Виражений в **чужих** термінах | Чужі |
| **Адаптер** | Реалізує наш інтерфейс, делегуючи до фасаду або чужого API напряму. Може робити кілька викликів за одне звернення | Наші → Чужі |
| **Транслятор** | Перетворює дані між формами — чужі DTO ↔ наші доменні моделі | Наші ↔ Чужі |

### Приклад: інтеграція з платіжною системою

**Інтерфейс** — бізнес-логіка знає, що є платіжний сервіс, але не знає деталей:

```python
# domain/ports.py
class PaymentGateway(Protocol):
    def charge(self, user_id: str, amount: Money) -> PaymentResult: ...
```

**Фасад** — покращує читабельність чужого SDK, залишаючись в його термінах:

```python
# infrastructure/acl/stripe_facade.py
class StripeFacade:
    def __init__(self, client: stripe.Client):
        self._client = client

    def create_charge(self, customer_id: str, amount_cents: int,
                      currency: str) -> stripe.Charge:
...
**Адаптер** — з'єднує наш інтерфейс з фасадом:

```python
# infrastructure/acl/stripe_adapter.py
class StripePaymentGateway(PaymentGateway):
    def __init__(self, facade: StripeFacade, translator: StripeTranslator):
        self._facade = facade
        self._translator = translator

    def charge(self, user_id: str, amount: Money) -> PaymentResult:
        charge = self._facade.create_charge(
            customer_id=user_id,
            amount_cents=self._translator.to_cents(amount),
            currency=amount.currency,
        )
        return self._translator.to_payment_result(charge)
```

**Транслятор** — перетворює дані між світами:

```python
# infrastructure/acl/stripe_translator.py
class StripeTranslator:
    ...
    @staticmethod
    def to_payment_result(charge: stripe.Charge) -> PaymentResult:
...
Якщо Stripe змінить API — оновлюємо фасад і транслятор. Бізнес-логіка не зачіпається.

### ACL між модулями

Той самий принцип працює і між модулями всередині моноліту:
...
Якщо Booking модуль перейменує `user_id` на `customer_id` — потрібно оновити лише транслятор, а не всю аналітику.

### Коли фасад не потрібен

Якщо чуже API вже достатньо зрозуміле (хороша бібліотека з чіткими типами) — фасад не потрібен. Адаптер може працювати напряму з чужим SDK. Фасад — це додатковий шар для випадків, коли чуже API незручне або занадто низькорівневе.

### Де розміщувати ACL?

ACL належить **споживачу** (downstream). Саме споживач вирішує, як транслювати чужу модель у свою:

```
src/
├── domain/
│   └── ports.py              # PaymentGateway (інтерфейс)
├── infrastructure/
│   └── acl/
│       ├── stripe_facade.py      # фасад (чужі терміни)
│       ├── stripe_adapter.py     # адаптер (наші → чужі)
│       └── stripe_translator.py  # транслятор (дані)
```
...
| Консистентність даних | Просто (одна БД, одна транзакція) | Середньо (shared DB або DB per module) | Складно ([eventual consistency](consistency.md#eventual-consistency)) |
