...
class Resource:
    def __init__(self, id: str):
        self._bookings: list[Booking] = []  # сотні/тисячі бронювань
...
class Booking:
    def __init__(self, id: str, user_id: str, resource_id: str, ...):
        self._user_id = user_id          # ID, не об'єкт User
        self._resource_id = resource_id  # ID, не об'єкт Resource
```
...
### 3. Одна транзакція — один агрегат

В ідеалі одна транзакція в БД змінює **один агрегат**. Якщо потрібно змінити кілька агрегатів у відповідь на одну дію — використовуйте [доменні події](domain-events.md) та eventual consistency.

### 4. Інваріанти визначають межі
